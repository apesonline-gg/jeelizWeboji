...
while True:
  ...
  nMorphsVertices = min(len(baseVertices), len(dVertices))

  print('baseVertices:', len(baseVertices))
  print('dVertices:', len(dVertices))

  if len(baseVertices) != len(dVertices):
    print('WARNING for morph %s: it has %d vertices but the base mesh has %d vertices' %(morphURL, len(dVertices), len(base['vertices'])))
